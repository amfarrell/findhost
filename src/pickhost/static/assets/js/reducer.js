...
function submit(state) {
  ...
  xhr({
    ...
    //TODO: Move this dispatch out of reducer.
...
export default function(state = initialState, action) {
  ...
  case 'CHANGE_NAME':
    return state.update('members', (members) =>
      updateName(members, action.index, action.name));
  case 'CHANGE_ADDRESS':
    return state.update('members', (members) =>
      updateAddress(members, action.index, action.address));
  case 'SUBMIT':
    return submit(state);
  case 'GEOCODE_FINISHED':
    return state.update('members', (members) =>
      applyGeocode(members, action.address, action.latlng));
  case 'PICKED':
